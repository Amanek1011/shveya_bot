...
from states import OtkStates


async def otk_start(call: types.CallbackQuery, state: FSMContext):
    """ОТК начинает работу"""
    await state.set_state(OtkStates.waiting_for_party_selection)
...
async def otk_party_selected(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(OtkStates.waiting_for_color_selection)
...
async def otk_color_selected(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(OtkStates.waiting_for_count)

    await call.message.edit_text(
        f"Контролер: {user_name}\n"
        f"Цвет: {color}\n\n"
        "Введите количество проверенных футболок:",
        reply_markup=get_cancel_keyboard()
    )
    await call.answer()


async def otk_count_handler(message: types.Message, state: FSMContext):
    try:
        count = int(message.text)
        data = await state.get_data()
        user_name = await user_service.get_user_name(message.from_user.id)

        await db.update_otk(data['material_id'], user_name, count)
...
async def otk_start_inline(message: types.Message, state: FSMContext):
    """Запуск работы для ОТК через меню"""
    ...
    await otk_start(fake_call, state)


async def otk_start_menu(message: types.Message, state: FSMContext):
    """Запуск работы для ОТК через меню (кнопку)"""
...
async def otk_continue_work(call: types.CallbackQuery, state: FSMContext, party_id: int = None):
    """Продолжить работу ОТК"""
    if not party_id:
